...
class BootstrapEmail {
	...
	static _gridHelper($) {
		...
		rows.each((rowIndex, _row) => {
			...
			const grid = $('<table><tbody></tbody></table>').addClass('grid');

			for (let gridIndex = 0; gridIndex < cols.length;) {
				let row = $('<tr>').addClass('row');

				// TODO: add support for different col amounts
				for (let rowCols = 0; gridIndex < cols.length; gridIndex++) {
					console.log(gridIndex);
					const col = $(cols[gridIndex]);
					...
					if (!lgSize) {
						...
						console.warn('No col sizes defined. Errors may happen');
					}
