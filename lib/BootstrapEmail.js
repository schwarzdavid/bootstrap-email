...
class BootstrapEmail {
	...
	static _marginHelper($) {
		...
		elements.each((i, _el) => {
			...
			const spacingVaraibles = BootstrapEmail._generateSpacingVariables(appendClasses);
	...
	static _generateSpacingVariables(classes) {
		classes = BootstrapEmail._ensureArray(classes);
		...
		classes.forEach(classname => {
			switch (classname.charAt(1)) {
				case 'l':
					output.spaceLeft = true;
					break;

				case 'r':
					output.spaceRight = true;
					break;
				...
				case 't':
					output.spaceTop = true;
					break;

				case 'b':
					output.spaceBottom = true;
					break;
