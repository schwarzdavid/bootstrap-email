...
class BootstrapEmail {
	...
	_compileHtml() {
		for (let template of this._templates) {
			...
			// BootstrapEmail._wrapElements($, '.float-left', 'align-left');
			// BootstrapEmail._wrapElements($, '.mx-auto', 'align-center');
			// BootstrapEmail._wrapElements($, '.float-right', 'align-right');
	...
	static _wrapContent($, tplName, {classes = [], attributes = {}, variables = {}} = {}){
		classes = BootstrapEmail._ensureArray(classes);
		attributes = BootstrapEmail._concatAttributes(attributes);

		// TODO: check this if it is necesssary
		if (attributes.class) {
			classes.push(...attributes.class.split(' '));
			delete attributes.class;
		}
	...
	static _wrapElement($, tplName, {classes = [], attributes = {}, variables = {}} = {}) {
		classes = BootstrapEmail._ensureArray(classes);
		attributes = BootstrapEmail._concatAttributes(attributes);
	...
	static _marginHelper($){
		...
		elements.each((i, _el) => {
			...
			BootstrapEmail._wrapElement(el, 'spacing', {classes: appendClasses, attributes: {'data-space-origin': 'margin'}});
		});
	}
	...
	static _paddingHelper($) {
		...
		elements.each((i, _el) => {
			...
			BootstrapEmail._wrapContent(el, 'spacing', {classes: appendClasses, attributes: {'data-spacing-origin': 'padding'}});
		});
	}
