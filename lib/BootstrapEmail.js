...
class BootstrapEmail {
	...
	constructor({
					...
					head = __defaults.head, containerWidthFallback = __defaults.containerWidthFallback,
					template,
					templates = [],
					content
				}) {
	...
	_compileHtml() {
		for (let template of this._templates) {
			...
			// Wrap layout classes
			// todo: handle containerConditionalCommentOption
			BootstrapEmail._replaceElement($, '.container', 'container', {variables: {}});
			BootstrapEmail._replaceElement($, '.container-fluid', 'container');

			// Replace horizontal lines and grid
			BootstrapEmail._replaceElement($, 'hr', 'hr', {classes: 'hr'});
			BootstrapEmail._replaceElement($, '*[class*="col"]', 'col', {attributes: {valign: 'top'}});
			...
			BootstrapEmail._replaceElement($, '.replace', 'replace', {attributes: {'data-foo': 'bar'}, classes: 'baum geil oida'});
	 ...
	 * @private
	 */
	static _replaceElement($, selector, tplName, {classes = [], attributes: _attributes = {}, variables = {}} = {}) {
	 ...
	 * @private
	 */
	static _wrapContent($, selector, tplName, {classes, attributes: _attributes = {}, variables = {}} = {}) {
		const elements = $(selector);
		...
		elements.each((i, _el) => {
			...
			//const attributes = BootstrapEmail._concatAttributes(el, _attributes) || '';
			const content = el.html();
			const template = cheerio(ejs.render(__templates[tplName], {variables, content, classes, attributes:_attributes}));
	 ...
	 * @private
	 */
	static _wrapElement($, selector, tplName, {attributes: _attributes = {}} = {}) {
		const elements = $(selector);
		...
		elements.each((i, _el) => {
			...
			if (tplName.indexOf('align-') >= 0 && el[0].name === 'table') {
				...
				const template = ejs.render(__templates[tplName], {content, attributes:_attributes});
